...
using Es.Auth;
using Es.IdentityServer;
using Es.IdentityServer.Services;
...
builder.Services.AddDbContext<AuthDbContext>(options => options.UseNpgsql(configuration.GetConnectionString("DbContextConnection")));
...
var idBuilder = builder.Services.AddIdentityServer(options =>
{
    ...
    options.IssuerUri = "http://es.identity-server";
